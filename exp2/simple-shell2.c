...
void simple_shell(char **argv) {
    ...
    while (TRUE) {
        ...
        //checking if it is background
        int bg = 0;
        if (input[size_input - 1] == '&') { // last token == '&', meaning that the command should run in the background
            bg = 1;
    ...
    // while (true)
        ...
        // if found
            ...
            // if pid == 0 
                // execv command
            //else
                //the parent process need to wait for the child process to finish        
